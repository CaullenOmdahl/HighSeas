import { b7 as Se, o as je, az as T, g as x, e as P, b8 as _t } from './DDdxbkju.js';
class ce {
	constructor(t, n) {
		((this.status = t),
			typeof n == 'string'
...
const U = globalThis.__sveltekit_1wmefjp?.base ?? '',
	Pt = globalThis.__sveltekit_1wmefjp?.assets ?? U,
	Ct = '1753890757224',
...
function Nt() {
	...
	async function r() {
		...
		try {
			...
			return (i && (e(!0), Qe.v(), clearTimeout(n)), i);
		} catch {
			return !1;
		}
	}
...
const Wt = 'x-sveltekit-invalidated',
	Yt = 'x-sveltekit-trailing-slash';
function te(e) {
...
const Zt = '/__data.json',
	Qt = '.html__data.json';
function en(e) {
	return e.endsWith('.html') ? e.replace(/\.html$/, Qt) : e.replace(/\/$/, '') + Zt;
}
const { tick: tn } = _t,
	nn = new Set(['icon', 'shortcut icon', 'apple-touch-icon']),
	D = Ke(Ye) ?? {},
	J = Ke(We) ?? {},
	N = { url: Be({}), page: Be({}), navigating: Se(null), updated: Nt() };
function Ue(e) {
	D[e] = fe();
}
function rn(e, t) {
...
async function kn(e, t, n) {
	(document.URL !== location.href && (location.href = location.href),
		...
		(A = history.state?.[F]),
		(I = history.state?.[Y]),
		A || ((A = I = Date.now()), history.replaceState({ ...history.state, [F]: A, [Y]: I }, '')));
	const r = D[A];
	function a() {
	...
	(n
		...
		: (await W({
				type: 'enter',
				url: Je(v.hash ? mn(new URL(location.href)) : location.href),
				replace_state: !0
			}),
			a()),
		pn());
...
async function it(e, t, n, r) {
	return W({
		...
		nav_token: r,
		accept: () => {
			(t.invalidateAll && (xe = !0), t.invalidate && t.invalidate.forEach(hn));
		}
	});
}
async function sn(e) {
...
function se({ url: e, params: t, branch: n, status: r, error: a, route: s, form: i }) {
	...
	const c = {
		...
		props: { constructors: Kt(n).map((l) => l.node.component), page: Ne(S) }
	};
...
async function ft({ id: e, invalidating: t, url: n, params: r, route: a, preload: s }) {
	...
	const m = f.map((g, y) => {
		const b = w.branch[y],
			k = !!g?.[0] && (b?.loader !== g[1] || Ge(p, u, h, l, b.server?.uses, r));
		return (k && (p = !0), k);
	});
	if (m.some(Boolean)) {
		try {
	...
	const E = f.map(async (g, y) => {
		...
		const b = w.branch[y],
			k = _?.[y];
		if ((!k || k.type === 'skip') && g[1] === b?.loader && !Ge(R, u, h, l, b.universal?.uses, r))
			return b;
	...
	for (let g = 0; g < f.length; g += 1)
		if (f[g])
			try {
				...
				let b = te(y),
					k;
				if (_?.includes(y)) ((b = y.status ?? b), (k = y.error));
				else if (y instanceof ce) k = y.body;
				else {
					if (await N.updated.check()) return (await tt(), await M(n));
...
async function de({ status: e, error: t, url: n, route: r }) {
	...
	try {
		...
		if (o instanceof Ee) return it(new URL(o.location, location.href), {}, 0);
		throw o;
	}
}
async function fn(e) {
	...
	try {
		const r = (async () => {
			let a =
				(await v.hooks.reroute({ url: new URL(e), fetch: async (s, i) => lt(s, i, e).promise })) ??
				e;
			if (typeof a == 'string') {
				const s = new URL(e);
...
function un(e) {
	return (
		yt(v.hash ? e.hash.replace(/^#/, '').replace(/[?#].+/, '') : e.pathname.slice(U.length)) || '/'
	);
}
function ie(e) {
...
async function W({
	...
	const u = await he(t, !1),
		l = e === 'enter' ? Oe(w, u, t, e) : ut({ url: t, type: e, delta: n?.delta, intent: u });
	if (!l) {
		(d(), $ === c && ($ = h));
		return;
	}
	...
	if (!_) {
		...
		_ = await dt(
			t,
			{ id: null },
			await V(new Re(404, 'Not Found', `Not found: ${t.pathname}`), {
				url: t,
				params: {},
				route: { id: null }
			}),
	...
	if (_.type === 'redirect')
		...
		else return (await it(new URL(_.location, t).href, {}, o + 1, c), !1);
	else _.props.page.status >= 400 && (await N.updated.check()) && (await tt(), await M(t));
	if (
		(on(),
		Ue(p),
		ot(m),
	...
	if (((O = null), (_.props.page.state = i), oe)) {
		const g = (await Promise.all(Array.from(an, (y) => y(l.navigation)))).filter(
			(y) => typeof y == 'function'
		);
		if (g.length > 0) {
			let y = function () {
...
function pn() {
	((history.scrollRestoration = 'manual'),
		...
		navigator.connection?.saveData || dn(),
		C.addEventListener('click', async (t) => {
			...
			const [f, d] = (v.hash ? r.hash.replace(/^#/, '') : r.href).split('#'),
				h = f === _e(location);
			if (a || (o.reload && (!h || !d))) {
				ut({ url: r, type: 'link' }) ? (X = !0) : t.preventDefault();
				return;
			}
			if (d !== void 0 && h) {
				...
				if (((q = !0), Ue(A), e(r), !o.replace_state)) return;
				q = !1;
			}
			(t.preventDefault(),
				await new Promise((u) => {
		...
		addEventListener('popstate', async (t) => {
			if (!ke) {
				if (t.state?.[F]) {
					...
					if (i === I && (rt || o)) {
						(a !== S.state && (S.state = a), e(s), (D[A] = fe()), r && scrollTo(r.x, r.y), (A = n));
						return;
					}
					const f = n - A;
					await W({
		...
		addEventListener('hashchange', () => {
			q &&
				((q = !1), history.replaceState({ ...history.state, [F]: ++A, [Y]: I }, '', location.href));
		}));
...
async function ht(e, t) {
	const n = new URL(e);
	((n.pathname = en(e.pathname)),
		e.pathname.endsWith('/') && n.searchParams.append(Yt, '1'),
		n.searchParams.append(Wt, t.map((s) => (s ? '1' : '0')).join('')));
...
`);
				...
				if (p.type === 'data')
					(p.nodes?.forEach((m) => {
						m?.type === 'data' && ((m.uses = pt(m.uses)), (m.data = f(m.data)));
					}),
						s(p));
...
export { kn as a, wn as l, S as p, N as s };
